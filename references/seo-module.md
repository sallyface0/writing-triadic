# 🔎 SEO 内容优化模块 — Writing Triadic v2.9
...
1. 当前模板属于 **SEO 适用类型**：
   - #1 技术文档 / #2 博客文章 / #4 评测 / #7 产品文案 / #17 即将新增: SEO 文章
...
└── Step 7: Phase 4 Reader 评分时增加 SEO 维度 (可选)
```
...
当 Reader 评审时，可选用 SEO 加权维度（默认权重 10%）：

| SEO 维度 | 在 Reader 6D 评分中的位置 | 权重 |
|----------|--------------------------|:----:|
| 关键词覆盖 | 追加到「内容完整性」维度 | +5% |
| 结构 SEO 友好度 | 追加到「结构逻辑」维度 | +3% |
| 可读性 | 追加到「表达流畅度」维度 | +2% |

**启用条件**：当模板属于 SEO 适用类型时，Reader 自动追加 SEO 权重。
